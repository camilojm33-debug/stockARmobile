...
class PurchasePaymentDetail(db.Model):
    ...
    purchase_order_id = db.Column(db.Integer, db.ForeignKey("purchase_orders.id", ondelete="CASCADE"), nullable=False, unique=True, index=True)
...
def register_routes(bp):
    ...
    def supplier_purchase_payment(supplier_id, purchase_id):
        _guard(); company_id = _company_id()
        if not company_id: abort(403)
        ...
        if detail is None:
            detail = PurchasePaymentDetail(purchase_order_id=purchase.id, company_id=company_id, payment_method="CUENTA_CORRIENTE", paid_amount=Decimal("0.00"), secondary_paid_amount=Decimal("0.00"), payment_term_days=0, payment_status="pendiente")
        if request.method == "POST":
            ...
            detail.payment_term_days, detail.due_date, detail.payment_status = term_days, due_date, status
...
def register_notification_extension():
    ...
    if getattr(notification_service, "_supplier_payment_extension", False): return
    ...
    def build_notifications_with_supplier_payments():
        ...
        if not getattr(current_user, "is_authenticated", False) or getattr(current_user, "role", None) != "admin": return items
        company_id = getattr(current_user, "company_id", None)
        if not company_id: return items
        try:
            from app import PurchaseOrder, Supplier, utcnow
            now = utcnow(); horizon = now.date() + timedelta(days=7)
            ...
            for detail, purchase, supplier in rows:
                ...
                elif days == 0: label, kind = "Vence hoy", "danger"
                else: label, kind = f"Vence en {days} día(s)", "warning"
                items.append({"type": kind, "title": "Pago a proveedor", "body": f"{supplier.name} · compra #{purchase.id} · {label} · pendiente ${float(remaining):.2f}.", "href": url_for("purchases.supplier_purchase_payment", supplier_id=supplier.id, purchase_id=purchase.id, company_id=company_id)})
