...
class MercadoPagoService:
    ...
    def _headers(self, *, include_idempotency=False, idempotency_key=None, access_token=None):
        ...
        if include_idempotency: headers["X-Idempotency-Key"]=(idempotency_key or str(uuid.uuid4())).strip()
        return headers
    def _request(self, method, path, *, payload=None, access_token=None, idempotency_key=None):
        ...
        try:
            with urlrequest.urlopen(req,timeout=self.REQUEST_TIMEOUT_SECONDS) as response: raw=response.read().decode(); status_code=response.getcode()
        except HTTPError as exc:
            raw_error=exc.read().decode(errors="ignore") if hasattr(exc,"read") else str(exc); raise RuntimeError(f"Mercado Pago rechazó la operación (HTTP {exc.code}). {raw_error[:700]}") from exc
        except (URLError,TimeoutError,socket.timeout) as exc: raise RuntimeError(f"Mercado Pago no respondió dentro de {self.REQUEST_TIMEOUT_SECONDS} segundos.") from exc
    ...
    def create_ai_order_checkout_preference(self, *, title, items, amount, currency, external_reference, company_id, user_id, quote_id, conversation_id, return_url, access_token=None):
        if amount<=0 or not items: raise ValueError("El pedido debe contener productos y un importe mayor a cero.")
        safe_items=[]
        for item in items:
            quantity=float(item.get("quantity") or 0); unit_price=float(item.get("unit_price") or 0)
            if quantity<=0 or unit_price<0: raise ValueError("Línea de pedido inválida.")
    ...
    def validate_webhook_signature(self, *, request_id, x_signature, data_id):
        ...
        parts=dict(part.split("=",1) for part in x_signature.split(",") if "=" in part); ts,v1=parts.get("ts"),parts.get("v1")
