...
class VendorOrderStatusTool(AgentTool):
    ...
    input_schema = {"type": "object", "properties": {"order_number": {"type": "string"}}, "additionalProperties": False}

    def execute(self, **kwargs):
        return VendorOrderService.get_customer_order_status(
            company_id=self.company_id, conversation_id=self._context["conversation_id"], order_number=str(kwargs.get("order_number") or "")
        )
...
class VendorCancelOrderTool(AgentTool):
    ...
    input_schema = {"type": "object", "properties": {"order_number": {"type": "string"}, "confirm": {"type": "boolean"}}, "additionalProperties": False}

    def execute(self, **kwargs):
        return VendorOrderService.cancel_order(
            company_id=self.company_id, conversation_id=self._context["conversation_id"], order_number=str(kwargs.get("order_number") or ""), confirm=bool(kwargs.get("confirm", False))
        )
...
class VendorCatalogTool(AgentTool):
    ...
    def execute(self, **kwargs):
        return VendorOrderService.list_catalog(company_id=self.company_id, query=str(kwargs.get("query") or ""), limit=int(kwargs.get("limit") or 20))
...
class VendorPromotionsTool(AgentTool):
    ...
    input_schema = {"type": "object", "properties": {"limit": {"type": "integer", "minimum": 1, "maximum": 24}}, "additionalProperties": False}
...
def install_vendor_followup_tools(runtime_cls) -> None:
    ...
    runtime_cls.agent_tool_names.setdefault("vendedor", set()).update({
        "consultar_pedido", "reintentar_pago", "cancelar_pedido", "ver_catalogo", "ver_promociones",
    })
