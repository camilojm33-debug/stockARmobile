...
def _shipping_plan(company_id: int, cart_total: Decimal, method: str) -> Dict[str, Any]:
    """Resolve the merchant-configured fixed shipping cost for Webchat."""
    normalized_method = _normalize_delivery_method(method)
    if normalized_method != "envio":
        return {
            "cost": Decimal("0.00"),
    ...
    if cost < 0:
        raise ValueError("El costo fijo de envío no puede ser negativo.")
    return {
        "cost": cost,
        "rate": Decimal("0.00"),
        "status": "confirmed",
        "source": "fixed",
        "reason": "Envío a domicilio · costo fijo del comercio.",
    }
...
class VendorOrderService:
    ...
    def create_pending_order(
        *,
        ...
        line_inputs, promotion_results = PromotionEngine.apply_to_lines(company_id=company_id, lines=line_inputs)
        promotion_by_product_id = {
            int(line["product"].id): line["promotion"]
        ...
        for index, item in enumerate(cart["items"]):
            ...
            promotion = promotion_by_product_id.get(product.id)
            discount = promotion.discount if promotion is not None else (_money(product.discount) * quantity).quantize(Decimal("0.01"))
