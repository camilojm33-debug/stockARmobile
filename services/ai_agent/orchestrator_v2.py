...
class AgentRuntime:
    ...
    def ensure_agent(cls, company_id: int, *, channel: str) -> Agent:
        agent = choose_agent(company_id, channel=channel)
        return agent
    ...
    def _history(cls, company_id: int, conversation_id: int, limit: int = 20):
        ...
        for row in reversed(rows):
            if row.role in {"user", "assistant"}:
                messages.append({"role": row.role, "content": str(row.content or "")})
        return messages
    ...
    def _execute_tool(cls, name: str, *, company_id: int, arguments: Dict[str, Any]):
        ...
        tool = tool_class(company_id=company_id)
    ...
    def process(cls, *, company_id: int, conversation_id: int, message: str, channel: str, sender_id=None, external_message_id=None, idempotency_key=None, metadata=None):
        ...
        messages = [{"role": "system", "content": prompt}] + cls._history(company_id, conversation.id, 20)
        ...
        response = cls.provider().generate(messages=messages, tools=cls._tool_definitions(), **kwargs)
        ...
        if tool_call:
            ...
            tool_messages = messages + [
                {"role": "assistant", "content": None, "tool_calls": [{"id": tool_call.get("id") or "call_1", "type": "function", "function": {"name": name, "arguments": json.dumps(args, ensure_ascii=False)}}]},
                {"role": "tool", "tool_call_id": tool_call.get("id") or "call_1", "content": json.dumps(result, ensure_ascii=False)},
            ]
            response = cls.provider().generate(messages=tool_messages, **kwargs)
            final_content = response.get("content") if isinstance(response, dict) else None
        ...
        assistant = ConversationMessage(
            ...
            metadata_json={"channel": channel, "agent_name": agent.name},
        )
