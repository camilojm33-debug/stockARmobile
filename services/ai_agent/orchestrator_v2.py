...
VENDOR_SYSTEM_PROMPT = """Sos el Vendedor 24 hs de StockARmobile. Consultá herramientas antes de afirmar precio o stock. No inventes información. Ayudá a elegir productos y cantidades. Podés preparar el pedido. Para pagar, indicá que el cliente debe usar el link de Mercado Pago generado por StockARmobile. Nunca afirmes que un pago fue recibido sin confirmación del webhook. Respondé en español argentino, breve y comercial."""
BUSINESS_SYSTEM_PROMPT = """Sos el Asistente empresarial de StockARmobile. Usá herramientas para consultar datos reales y nunca inventes cifras. Podés informar ventas, ticket promedio, stock crítico, precios, existencias y clientes. Las acciones que cambien dinero, stock, clientes o ventas requieren una operación explícita y segura."""
...
class VendorAddTool(AgentTool):
    ...
    description = "Agrega un producto por nombre, código o consulta y cantidad al carrito."
    input_schema = {"type":"object","properties":{"product_query":{"type":"string"},"quantity":{"type":"number","minimum":0.01}},"required":["product_query","quantity"],"additionalProperties":False}
...
class VendorRemoveTool(AgentTool):
    ...
    description = "Quita del carrito un producto por nombre o código."
    input_schema = {"type":"object","properties":{"product_query":{"type":"string"}},"required":["product_query"],"additionalProperties":False}
...
class VendorOrderPreviewTool(AgentTool):
    ...
    description = "Prepara un pedido pendiente y genera un link seguro de pago Mercado Pago; no confirma el cobro."
    input_schema = {"type":"object","properties":{"customer_name":{"type":"string"}},"additionalProperties":False}
...
class AgentRuntime:
    ...
    def _tool_definitions(cls):
        return [{"type":"function","function":{"name":n,"description":getattr(t,"description",""),"parameters":getattr(t,"input_schema",{"type":"object","properties":{}})}} for n,t in cls.tool_registry.items()]
    @classmethod
    def _execute_tool(cls,name,*,company_id,arguments,context):
        tool_class=cls.tool_registry.get(name)
        if tool_class is None: return {"success":False,"error":"tool_not_found"}
        if "company_id" in arguments: return {"success":False,"error":"company_id_not_allowed"}
        return tool_class(company_id=company_id,**context).execute(**arguments)
    @classmethod
    def process(cls, *, company_id, conversation_id, message, channel, sender_id=None, external_message_id=None, idempotency_key=None, metadata=None):
        ...
        if conversation is None: raise ValueError("Conversation no encontrada para la empresa.")
        ...
        incoming=ConversationMessage(conversation_id=conversation.id,company_id=company_id,sender_type="user",sender_id=sender_id,role="user",content=str(message),content_type="text",external_message_id=external_message_id,idempotency_key=idempotency_key,trace_id=str(uuid.uuid4()),metadata_json=metadata or {})
        ...
        provider=cls.provider(); response=provider.generate(messages=messages,tools=cls._tool_definitions(),**kwargs)
        ...
        if tool_call:
            name=tool_call.get("name"); args=tool_call.get("arguments") or {}
            context={"conversation_id":conversation.id,"customer_phone":(metadata or {}).get("from") or "","actor_user_id":sender_id}
            result=cls._execute_tool(name,company_id=company_id,arguments=args,context=context)
        ...
        assistant=ConversationMessage(conversation_id=conversation.id,company_id=company_id,sender_type="agent",sender_id=agent.id,role="assistant",content=str(final_content),content_type="text",external_message_id=None,idempotency_key=None,trace_id=incoming.trace_id,metadata_json={"channel":channel,"agent_name":agent.name})
