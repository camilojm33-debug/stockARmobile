...
from services.ai_agent.tools.business_metrics import ContarClientesTool, ContarProductosTool, ProductosMasVendidosTool, ProductosSinVentasRecientesTool, ResumenVentasTool, StockCriticoTool
...
class VendorAddTool(AgentTool):
    ...
    def execute(self, **kwargs):
        return VendorOrderService.update_cart(company_id=self.company_id, conversation_id=self._context["conversation_id"], items=[kwargs])
...
class VendorRemoveTool(AgentTool):
    ...
    def execute(self, **kwargs):
        return VendorOrderService.remove_from_cart(company_id=self.company_id, conversation_id=self._context["conversation_id"], product_query=kwargs["product_query"])
...
class AgentRuntime:
    ...
    agent_tool_names = {
        "asistente": {"buscar_producto", "consultar_stock", "buscar_cliente", "contar_clientes", "contar_productos", "resumen_ventas", "productos_mas_vendidos", "productos_sin_ventas_recientes", "stock_critico"},
        "vendedor": {"buscar_producto", "consultar_stock", "buscar_cliente", "carrito_vendedor", "agregar_al_carrito", "quitar_del_carrito", "preparar_pedido"},
        "analista": {"resumen_ventas", "productos_mas_vendidos", "productos_sin_ventas_recientes", "stock_critico", "contar_clientes", "comparar_ventas", "clientes_inactivos"},
        "marketing": {"buscar_producto", "consultar_stock", "buscar_cliente", "clientes_inactivos", "productos_promocionables", "preparar_campana"},
    }
    ...
    def _provider_model(cls, provider, config):
        """Avoid reusing a model from a different provider (e.g. gpt-* with Gemini)."""
    ...
    def process(
        ...
        *,
        ...
        prompt = {"vendedor": VENDOR_SYSTEM_PROMPT, "asistente": BUSINESS_SYSTEM_PROMPT, "analista": ANALYST_SYSTEM_PROMPT, "marketing": MARKETING_SYSTEM_PROMPT}[agent_key]
        ...
        response = provider.generate(messages=messages, tools=cls._tool_definitions(agent_key), **kwargs)
        ...
        assistant = ConversationMessage(
            ...
            content_type="text",
            trace_id=trace_id,
            metadata_json={"channel": channel, "agent_name": agent.name, "agent_key": agent_key},
        )
