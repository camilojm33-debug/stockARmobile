...
with stock_app.app.app_context():
    ...
    u = User(username='superadmin', email='superadmin@test.local', role='superadmin')
    u.set_password('admin123')
    db.session.add(u)
    db.session.commit()

    client = stock_app.app.test_client()
    r = client.post('/auth/login', data={'username': 'superadmin', 'password': 'admin123'})
    print('login', r.status_code, r.headers.get('Location'))
    r2 = client.get('/superadmin/billing')
